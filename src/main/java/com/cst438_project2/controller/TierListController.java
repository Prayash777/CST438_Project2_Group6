...
public class TierListController {
    ...
    public String saveTierList(HttpSession session, @RequestParam("category") String category,
        ...
        try {
            ...
            return "redirect:/view-tier-list?category=" + category + "&userId=" + userId;
 ...
 @GetMapping("/view-tier-list")
 public String viewTierList(@RequestParam("category") String category, 
    ...
    try {
        ...
        if (categoryEntity == null || user == null) {
            model.addAttribute("error", "Invalid category or user.");
            return "error"; // Show an error page if user or category doesn't exist
        }

        // Retrieve the saved tier list
        List<TierEntry> tierEntries = tierListService.findTierEntriesByUserAndCategory(user, categoryEntity);
        model.addAttribute("tierEntries", tierEntries);
        model.addAttribute("category", categoryEntity.getCategoryName());
        model.addAttribute("user", user);

        return "tierListView"; // Render the saved tier list
    } catch (Exception e) {
        model.addAttribute("error", "An error occurred while loading the tier list.");
